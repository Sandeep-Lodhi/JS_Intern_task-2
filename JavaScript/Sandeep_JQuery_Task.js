...
function result() {
    ...
    const tabbody = document.createElement("tr");
    // tabbody.className = "";
    tabbody.innerHTML = `
        <td>${firstname}</td>
        <td>${lastname}</td>
        ...
        <td>${email}</td>
        <td>${address}</td>
        <td><button type="button" onclick="view(${inputdata.ID});" class="btn btn-primary  btn-sm ">Edit</button><button type="button" onclick="view(${inputdata.ID});" class="btn btn-primary btn-sm ms-2"><i class="fa-solid fa-eye"></i></button></td>`
    // data++;
    const div = document.getElementById("tbody");
    div.appendChild(tabbody);
    tabCount++;
